...
float whash(vec2 a) { return (hash(a) - .5) * WATER_HEIGHT; }
vec3 dperlin(vec2 a) {
  vec2 n1 = dinter(whash(floor(a)), whash(floor(a + vec2(1, 0))), fract(a.x));
  vec2 n2 =
      dinter(whash(floor(a + vec2(0, 1))), whash(floor(a + 1.)), fract(a.x));
  return vec3(dinter(n1.x, n2.x, fract(a.y)), dinter(n1.y, n2.y, fract(a.y)).x)
      .xzy;
}
float perlin(vec2 a) {
  float n1 = inter(whash(floor(a)), whash(floor(a + vec2(1, 0))), fract(a.x));
  float n2 =
      inter(whash(floor(a + vec2(0, 1))), whash(floor(a + 1.)), fract(a.x));
...
#define speed 3.3
vec3 dfbm(vec2 p) {
  vec3 n = dperlin(-p + frameTimeCounter * .2 * speed) +
           dperlin(p + frameTimeCounter * .3 * speed);
  float tt = .5;
...
float fbm(vec2 p) {
  float n = perlin(-p + frameTimeCounter * .2 * speed) +
            perlin(p + frameTimeCounter * .3 * speed);
  float tt = .5;
  for (int i = 1; i++ < WAVE_NOISE_OCTAVES;) {
    float s = float(i);
    p *= mat2(cos(i * tt), sin(i * tt), -sin(i * tt), cos(i * tt));
...
vec3 parallaxposw(vec3 p, vec3 rd) {
  ...
  for (int i = 0; i < 150 && pc.z > height; i++) {
    pc.xy += rstep.xy *
             clamp((pc.z - height) / (step.z * .2 / (.05 - rd.z)), 0., 1.);
    pc.z += rstep.z;
...
float map(vec2 uv) { return -400. * (normaps(uv) + normaps(-uv)); }
vec3 raymarche(vec3 ro, vec3 rd) { // hybrid voxel raymarcher.
  ro *= 10.;
  vec2 zu = vec2(0, 1);
  vec2 p = floor(ro.xy);
  vec2 s = sign(rd.xy);
  vec2 td = s / rd.xy, tm = max(s, 0.) * td - (fract(ro) / rd).xy, tm2 = tm;
  vec3 n = zu.xyx;
  for (int i = 0; i++ < it;) {
    ...
    if (pd > 0.) {
      ...
      float md = min(ds.x < 0. ? 1e6 : ds.x,
                     ds.y < 0. ? 1e6 : ds.y); // takes min and discard negatives
...
vec4 gettex(sampler2D t, vec2 v) { return texture2DGradARB(t, v, dcdx, dcdy); }
...
#include "lib/lightcol.glsl"
  ...
  if (iswater) {
    ...
    float trans = t.a;
    fragData0 = tintColor;
// fragData0.rgb=vec3(1.-(WATER_TEXTURE_RESOLUTION*length(tc-midTexCoord)));
#ifdef WATER_TEXTURE
    fragData0 =
        vec4(srgbToLinear(c * fragData0.rgb * .5) * lightCol * lmcoord.y / 256.,
             trans);
    fragData1 = vec4(1, 1., srgbToLinear(c).r, 1);
#else
    fragData0 = vec4(0, 0, 0, 0.01);
    fragData1 = vec4(1, 1., 1., 1.);
#endif

    vec3 n = iswater ? tnb * normalize(normap(p.xz).xyz) : normal;
#ifdef PILLAR_WATER
    n = iswater ? tnb * raymarche(vec3(position.xz, 0.), normalize(vpos) * tbn)
                : n;
#endif
    fragData2 = vec4(normalize(n) * .25 + .5, 1.);
#if PBR_FORMAT == labPBRv1_3
    fragData3 = vec4(.9, .366, 0., 1.);
#else
    fragData3 = vec4(.9, .134, 0., 1.);
#endif
  } else {
    vec2 uv = texcoord.st;
...
#endif
    fragData0 = gettex(texture, uv);
    fragData0.rgb =
        srgbToLinear(fragData0.rgb * tintColor.rgb) * .5; //*0.+blocklightdir;
    // fragData0.a=1.;
    // fragData0.rg = texres;gl_FragData[0.b=0.;
    // fragData0.rgb=vec3(getpa(texture,uv),0.);
    fragData3 = vec4(PBRdata.rgb, 1);
    vec3 n = normal;
#ifdef NORMAL_MAPPING
    n = tbn * normalize(gettex(normals, uv).rgb * 2. - 1.);
#endif
    fragData2 = vec4(n * .5 + .5, 1.);

    // fragData0=vec4(PBRdata.rgb,1.);

#include "lib/ambcol.glsl"

    fragData0.rgb *=
        mix((ambientCol * lm.y * ambi + TorchColor * lm.x),
            diffuse(normalize(vpos), (normalize(shadowLightPosition)), n,
                    1. + 0. * pow(1. - PBRdata.r, 2.)) *
                    lightCol * shadow3(world2cam(p)) +
                ambientCol * lm.y * ambi + TorchColor * lm.x,
            fragData0.a);

    fragData1 = vec4(1, 0., 1., 1.);
  }
  fragData4 = vec4(lm, 0., 1.);
}
