...
vec3 dperlin(vec2 a) {
  ...
  vec3 ret =
      vec3(dinter(n1.x, n2.x, fract(a.y)), dinter(n1.y, n2.y, fract(a.y)).x);
  return ret.xzy;
}
float perlin(vec2 a) {
...
// gl_FragData[0].rgb=vec3(1.-(WATER_TEXTURE_RESOLUTION*length(tc-midTexCoord)));
#ifdef WATER_TEXTURE
    {
      vec3 fd = c * gl_FragData[0].rgb * .5;
      gl_FragData[0].rgb = srgbToLinear(fd) * lightCol * lmcoord.y / 256.;
    }
    ...
    gl_FragData[0].rgb = srgbToLinear(gl_FragData[0].rgb * tintColor.rgb) * .5;
    // gl_FragData[0].a=1.;
